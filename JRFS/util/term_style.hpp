...
/// \brief Class to help stylize the stream outputs.
...
public:
    ...
    inline void line(char pattern = '-', int n = 0, std::ostream& os = std::cout) const
    {
        if (0 == n)
            n = get_term_length() - 2;
    ...
    /// \param os The output stream.
    inline static void padding(int n, char pattern = ' ', std::ostream& os = std::cout)
    {
        for (int i = 0; i < n; ++i)
            os << pattern;
    };
    ...
    /// \param os The output stream.
    inline void string_field(const std::string& res, size_t max_len = 0, std::ostream& os = std::cout) const
    {
        if (0 == max_len)
            max_len = res.size();
...
constexpr TermStyle<0> RED(Color::RED), BLUE(Color::BLUE), GREEN(Color::GREEN), BLACK(Color::BLACK),
    YELLOW(Color::YELLOW), PURPLE(Color::PURPLE), CYAN(Color::CYAN), WHITE(Color::WHITE), DEF(Color::DEFAULT),
    CLEAN(Color::CLEAN);
